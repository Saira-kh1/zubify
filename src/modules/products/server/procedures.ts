  ...
  getMany: baseProcedure
    ...
    .query(async ({ ctx , input}) => {
      ...
      if(input.tenantSlug) {
        ...
        // if we are loading products for public stoefront (no tenantSlug)
      ...
      if(input.category){
        const categoriesData = await ctx.db.find({
          ...
          limit: 1,
          depth: 1,  // Because of the depth: 1, we are confident "doc" will be type of "Category" these will not have further subcategories
          pagination: false,
          where:{
            slug:{
    ...
    if(input.tags && input.tags.length > 0){
      ...
      const data = await ctx.db.find({
        collection: 'products',
        depth: 2, // Include category up to 1 level deep //populate category and "image" and tenant
        where,
        sort,
        page: input.cursor,
        limit: input.limit,
      ...
      data.docs.map(async (doc) => {
          ...
          return{
            ...
            reviewRating:
              ...
              ? 0
              : reviewData.docs.reduce((acc, review) => acc + review.rating, 0) /reviewData.totalDocs
          }
      })
     );
